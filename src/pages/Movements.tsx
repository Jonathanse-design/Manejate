    ...
    <div className="two-column movements-layout">
      <section>
        ...
        <div className="toolbar">
          <Search size={18} />
          <input onChange={(event) => setQuery(event.target.value)} placeholder="Buscar por categoría, descripción o nota" value={query} />
        </div>
        ...
        <div className="finance-table-wrap">
          <table className="finance-table">
            ...
            <tbody>
              {filtered.map((item) => (
                <tr key={item.id}>
                  <td className="left-cell date-cell">{humanDate(item.date)}</td>
                  <td className="left-cell description-cell">
                    <strong>{item.description}</strong>
                    {item.note && <span>{item.note}</span>}
                  </td>
                  <td className="left-cell">{item.category}</td>
                  <td className="center-cell"><span className={`type-pill ${item.kind}`}>{typeLabel[item.kind]}</span></td>
                  <td className="center-cell">{item.method}</td>
                  <td className="left-cell">{productName(item.linkedProductId)}</td>
                  <td className={`amount-cell ${item.kind}`}>
                    {item.kind === 'income' ? '+' : '-'}{formatMoney(item.amount, currency, data.settings.privacyMode)}
                  </td>
                  <td className="center-cell">
                    <button aria-label="Eliminar movimiento" className="icon-button" onClick={() => deleteTransaction(item.id)} type="button">
                      <Trash2 size={17} />
                    </button>
                  </td>
                </tr>
          ...
          <div className="movement-card-list">
            {filtered.map((item) => (
              <article className="movement-card" key={item.id}>
                ...
                <p>{item.category} · {typeLabel[item.kind]} · {item.method}</p>
                <p>{productName(item.linkedProductId)}</p>
      ...
      <aside>
        ...
        <TransactionForm mode={active.mode} onSave={addTransaction} products={active.products} />
      </aside>
    </div>
  );
};
